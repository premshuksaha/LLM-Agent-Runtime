class llmagent {
    ...
    async init() {
        try {
            ...
            console.log(`LLM Agent Runtime v${this.version} initialized successfully`);
        } catch (error) {
            console.error('Failed to initialize LLM Agent Runtime:', error);
    ...
    addMessage(role, content, conversationId) {
        try {
            ...
            if (role !== 'system' && typeof content === 'string') {
                ...
                if (!conversation.title || conversation.title === 'New Conversation') conversation.title = content || 'Conversation';
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'LLM Agent Runtime', system: 'System', tool: 'Tool' }[message.role] || message.role;
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to LLM Agent Runtime! How can I assist you?');
        }
    }
